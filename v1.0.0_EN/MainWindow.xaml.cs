...
namespace VeloxDNS_Complete
{
    public partial class MainWindow : Window
    {
        ...
        private void LoadNetworkAdapters()
        {
            ...
            AdapterComboBox.Items.Add("All adapter");
        ...
        private void SetDns_Click(object sender, RoutedEventArgs e)
        {
            ...
            MessageBox.Show("DNS set.", "Success");
        }
        ...
        private void SetAuto_Click(object sender, RoutedEventArgs e)
        {
            ...
            MessageBox.Show("DNS reset.", "Success");
        }
        ...
        private void SaveProfile_Click(object sender, RoutedEventArgs e)
        {
            ...
            SaveProfiles();
            LoadProfiles();
            MessageBox.Show("Profile saved.", "info");
        }
        ...
        private void ShowAdapterInfo_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(adapter) || adapter == "All adapter")
            {
                MessageBox.Show("Please select a single adapter to display information.", "Notice");
                return;
            }
            ...
            foreach (var nic in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                sb.AppendLine($"MAC-Address: {BitConverter.ToString(nic.GetPhysicalAddress().GetAddressBytes())}");
                ...
                sb.AppendLine($"DNS (IPv4 1): {(string.IsNullOrEmpty(dnsV4_1) ? "No" : dnsV4_1)}");
                sb.AppendLine($"DNS (IPv4 2): {(string.IsNullOrEmpty(dnsV4_2) ? "No" : dnsV4_2)}");
                sb.AppendLine($"DNS (IPv6 1): {(string.IsNullOrEmpty(dnsV6_1) ? "No" : dnsV6_1)}");
                sb.AppendLine($"DNS (IPv6 2): {(string.IsNullOrEmpty(dnsV6_2) ? "No" : dnsV6_2)}");
            }
        ...
        private void TestSingleDns(string address, StringBuilder sb)
        {
            ...
            try
            {
                ...
                if (reply.Status == IPStatus.Success)
                    sb.AppendLine($"{address} reachable (ping: {reply.RoundtripTime} ms)");
                else
                    sb.AppendLine($"{address} not reachable (status: {reply.Status})");
            }
            catch (Exception ex)
            {
                sb.AppendLine($"{address} Mistake: {ex.Message}");
            }
        }
